...
async function smtpCommand(writer, reader, command, expectedCodes) {
...
__name(smtpCommand, "smtpCommand");
async function sendGmailOtp(to, otp, smtpUser, smtpPass) {
...
__name(sendGmailOtp, "sendGmailOtp");
var onRequestPost = /* @__PURE__ */ __name(async (context2) => {
  try {
    ...
    if (!context2.env.SMTP_USER || !context2.env.SMTP_PASS) {
      console.error("[SFF] SMTP binding check:", JSON.stringify({ SMTP_USER: !!context2.env.SMTP_USER, SMTP_PASS: !!context2.env.SMTP_PASS }));
      return new Response(
        JSON.stringify({
          success: false,
    ...
    const existingRaw = await context2.env.SFF_OTP_STORE.get(key);
    if (existingRaw) {
      try {
        const existing = JSON.parse(existingRaw);
        if (existing.lastSentAt && Date.now() - Number(existing.lastSentAt) < 3e4) {
          return new Response(
            JSON.stringify({
              success: false,
    ...
    const now = Date.now();
    await context2.env.SFF_OTP_STORE.put(
      key,
      JSON.stringify({
        otp,
        expiresAt: now + 6e5,
    ...
    try {
      await sendGmailOtp(
        email,
        otp,
        context2.env.SMTP_USER,
        context2.env.SMTP_PASS
      );
    } catch (smtpError) {
      console.error(
      ...
      await context2.env.SFF_OTP_STORE.delete(key);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    console.error("[SFF] Registration OTP error:", error3);
    return new Response(
      JSON.stringify({
        success: false,
...
var onRequestPost2 = /* @__PURE__ */ __name(async (context2) => {
  try {
    ...
    const key = "registration_otp:" + encodeURIComponent(email);
    const record = await context2.env.SFF_OTP_STORE.get(key, "json");
    if (!record) {
      return new Response(JSON.stringify({
        success: false,
...
// ../.wrangler/tmp/pages-038Zdb/functionsRoutes-0.13150369669897521.mjs
var routes = [
  {
